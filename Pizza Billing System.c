...
break;
   ...
   struct Pizza p;
   FILE *fptr;
   if ((fptr = fopen("vasvi.bin","ab+")) == NULL){
...
int check(int no)
{
	struct Pizza p; int n=0;
	   FILE *fptr;
   if ((fptr = fopen("vasvi.bin","rb")) == NULL){
...
{
	char newname[]="vasvi.bin",oldname[]="vas.bin";
...
void editbill(){
    ...
    printf("\r                                                     \r");
		...
		break;
		case '3':
				printf("\nSure to change Qantity of pizza?(Y/N)\n");
		fflush(stdin);
		while(isspace(c)!=0||c=='\n'){
				c=getche();
		}
...
void modsize()
{
	...
	fclose(fp);
      onlycart();
	int sno; char ch;
	printf("Enter S.No of pizza you would like to change size of :\n");
	while((scanf("%d%c",&sno,&ch) !=2 || ch!='\n')|| sno<=0)
   {
   printf("Invalid Input.Try again!\n");
   fflush(stdin);
...
FILE *f1,*f2,*f3;
	f1=fopen("temp_bill.bin","rb");
	f2=fopen("temp_bill2.bin","wb");
	f3=fopen("vasvi.bin","rb");
	struct Bill b; struct Pizza p;
...

{
 ...
 while(ftell(f3)!=pos2)
 {
 	fread(&p,sizeof(struct Pizza),1,f3);
       if(p.sno==b.psno)
	   break;
...
void modqty()
{
	...
	fclose(fp);
      onlycart();
	int sno; char ch;
	printf("Enter S.No of pizza you would like to change qty of :\n");
	while((scanf("%d%c",&sno,&ch) !=2 || ch!='\n')|| sno<=0)
   {
   printf("Invalid Input.Try again!\n");
   fflush(stdin);
...
if(n==0)
 ...
 printf("\nQty modified sucessfullly.\n");
	printf("Change quantity of another pizza?(Y/N)\n");
		fflush(stdin);
		while((ch=getche())!='Y'&& ch!='N'&& ch!='y'&& ch!='n')
		{
		printf(" is an invalid Input.Enter again. ");
